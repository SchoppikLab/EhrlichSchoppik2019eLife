            ...
            if isempty(find(abs(displacement) > maxInstDisp,1))
                ...
                if isempty(find(t > maxDeltaT, 1))
                    ...
                    if isempty(find(abs(angvel) > maxAngVel,1))                                              
                        ...
                        %only keep epochs in which fish is pointed in the
                        %direction it moves. this excludes fish with error
                        %detecting head postion.
                        if (mean(headx) - mean(x))*(x(end)-x(1)) >= 0 %if pointed and moving right, both positive; if left, both negative; mismatch yields negative product
